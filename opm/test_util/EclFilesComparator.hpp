...
/// Prototype for eclipse keyword struct, defined in the ERT library
...
/*! ECLFilesComparator opens ECLIPSE files (unified restart, initial and RFT in addition to grid file) from two simulations.
 *  This class has only the functions printKeywords() and printKeywordsDifference(), in addition to a couple of get-functions: the comparison logic is implemented in the subclasses RegressionTest and IntegrationTest. */
...
/*!This class inherits from ECLFilesComparator, which opens and closes the input cases and stores keywordnames. The three public functions gridCompare(), results() and resultsForKeyword() can be invoked to compare griddata
...
class RegressionTest: public ECLFilesComparator {
    private:
        ...
        // Functions which compares data at specific occurrences and for a specific keyword type. The functions takes two occurrence inputs to also be able to
...
/*!This class inherits from ECLFilesComparator, which opens and closes the input cases and stores keywordnames. The three public functions equalNumKeywords(), results() and resultsForKeyword() can be invoked to compare griddata
 * or keyworddata for all keywords or a given keyword (resultsForKeyword()). */
class IntegrationTest: public ECLFilesComparator {
    private:
        // Vector of cell volumes in second input case (indexed by global index)
        std::vector<double> cellVolumes;

        // Keyword values for all cells at first occurrence (index by global index)
        std::vector<double> initialCellValues;
