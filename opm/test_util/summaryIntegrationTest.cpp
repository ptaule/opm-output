...
void IntegrationTest::getIntegrationTest(){
    ...
    while(ivar < stringlist_get_size(keysShort)){
        ...
        for (size_t jvar = 0; jvar < stringlist_get_size(keysLong); jvar++){
...
void IntegrationTest::checkForKeyword(std::vector<double> timeVec1, std::vector<double> timeVec2, const char* keyword){
...
int IntegrationTest::checkDeviation(Deviation deviation){
...
void IntegrationTest::findGreatestErrorRatio(WellProductionVolume volume, double &greatestRatio, const char* currentKeyword, std::string &keywordWithGreatestErrorRatio){
    if (volume.total != 0 && (volume.total - volume.error > getAbsTolerance()) ){
        if(volume.error/volume.total > greatestRatio){
            ...
            keywordWithGreatestErrorRatio = currentKeywordStr;
        }
    }
}
...
WellProductionVolume IntegrationTest::getWellProductionVolume(const char * keyword){
    ...
    double error = integrateError(referenceVec, referenceDataVec, checkVec, checkDataVec);
...
WellProductionVolume IntegrationTest::getSpecificWellVolume(std::vector<double> timeVec1, std::vector<double> timeVec2, const char* keyword){
    std::vector<double> dataVec1, dataVec2;
    getDataVecs(dataVec1,dataVec2,keyword);
    chooseReference(timeVec1, timeVec2,dataVec1,dataVec2);
...
double IntegrationTest::integrate(std::vector<double> *timeVec, std::vector<double> *dataVec){
    double totalSum = 0;
    double width, height;
...
double IntegrationTest::integrateError(std::vector<double> *timeVec1, std::vector<double> *dataVec1, std::vector<double> *timeVec2, std::vector<double> *dataVec2){
